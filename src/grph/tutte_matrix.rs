...
pub fn max_matching_size<const M: u64>(n: usize, es: &[[usize; 2]], rng: &mut impl Rng) -> u64 {
...
pub fn max_matching<const M: u64>(n: usize, es: &[[usize; 2]]) -> Vec<[usize; 2]> {
    ...
    for _ in 0..m / 2 {
        ...
        let pivot_inv_ij = inverse_euclidean::<M>(pivot_val_ij);
        let pivot_inv_ji = M - pivot_inv_ij;
        for i in 0..m {
            if !has[i] {
                continue;
            }
            if a_inv[(i, fj)] != 0 {
                let multiplier = (a_inv[(i, fj)] * pivot_inv_ij) % M;
                for j in 0..m {
                    a_inv[(i, j)] = (a_inv[(i, j)] + M - (a_inv[(fi, j)] * multiplier) % M) % M;
                }
            }
            if a_inv[(i, fi)] != 0 {
                let multiplier = (a_inv[(i, fi)] * pivot_inv_ji) % M;
                for j in 0..m {
                    a_inv[(i, j)] = (a_inv[(i, j)] + M - (a_inv[(fj, j)] * multiplier) % M) % M;
                }
            }
        }
    }
...
mod tests {
    ...
    fn test_matching_size_consistency() {
        ...
        assert_eq!(matching.len() as u64, size);
        assert!(is_valid_matching(4, &edges, &matching));
    }
